# SummResults
Results processing for the SUMM

MapRun results: 
https://maprunners.weebly.com/access-to-results.html

https://maprunners.weebly.com/uploads/6/8/6/6/6866500/maprun_api_for_public_results_info_v2.1.pdf

https://p.fne.com.au:8886/resultsGetPublicForEventv2?eventName=<Full event name>
(ie a HTTP GET request method)


The SUMM 2026 events:
SUMM Day 1 v2 ScoreP420
SUMM Day 2 v2 ScoreP360

Event site:
https://www.trionium.com/SUMM/

Handicap rules:
If either team member is aged 45 or over then the team qualifies for a handicap. This is based on real ages. If a team does not qualify then the handicap is zero. The effective age of a team member is their real age if male or their real age plus 10 if female. Take the maximum effective age of the team and subtract 45 from this to get the handicap percentage. The handicap result of each team is their standard result increased by the handicap percentage.

## TODO
* allow a team member to not exist in the maprun results but still use for handicapping
* assign teams to a specific event (members from one year to the next might change)
